    ...
    where TCommand : ICommand<TOutput>
{
    ...
    public ValidationCommandDecorator(ILogger<ValidationCommandDecorator<TCommand, TOutput>> logger,
        IEnumerable<IValidator<TCommand>> validators, IAsyncCommandHandler<TCommand, TOutput> handler)
    {
        _logger = logger;
...
public sealed class ValidationQueryDecorator<TQuery, TResult> : IAsyncQueryHandler<TQuery, TResult> where TQuery : IQuery<TResult>
{
    ...
    public ValidationQueryDecorator(ILogger<ValidationQueryDecorator<TQuery, TResult>> logger,
        IEnumerable<IValidator<TQuery>> validators, IAsyncQueryHandler<TQuery, TResult> handler)
    {
        _logger = logger;
        _validators = validators;
        _handler = handler;
